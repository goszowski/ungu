...
function updatenode($request) {
	...
	foreach ($_keys as $fieldShortname) {
		...
		//if (in_array($fieldShortname, array("parent_id", "name", "absolute_path"))) {
			//include_once("special_incs/node_update.php");
		//}
		$fieldList[$fieldDef->shortname] = &$field;
	}
	$NodeName = (string)$request->getParameter("NodeName");
	...
	if (sizeof($fieldErrors) == 0) {
		...
		if ($currentLoggedUser->group->isModerator) {
			...
			*/
			...
			//kazancev - start
			/*
			$__fields = array_keys($fieldList);
			if (array_intersect($__fields, array("parent_id", "name", "absolute_path"))) {
				//_dump($fieldList);
				foreach ($fieldList as $f_name => $f_obj) {
					if (in_array($f_name, array("parent_id", "name", "absolute_path"))) {
						//_dump($f_name);
						//_dump($node->$f_name);
						$fieldList[$f_name]->value = $node->$f_name;
					}
				}
...
function grp_edit () {
	//_dump($_POST["grp"]);
	if (is_array($_POST["grp"])) foreach ($_POST["grp"] as $fieldname => $data) {
		if (is_array($data)) foreach ($data as $id => $value) {
			//_dump($value);
			$tmp_node = &Node::findById($id);
			//_dump($tmp_node);
			if (is_object($tmp_node)) {
				$tmp_node->getField($fieldname)->setValue($value);
				$tmp_node->store();
			}
		}
	}
	$forwardUrl = "/admin/nodes.php?do=main&id=" . $_POST["parent_id"];
	Header("Location: ".$forwardUrl);
	return;
}
