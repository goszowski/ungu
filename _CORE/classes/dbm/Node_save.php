...
class Node {
	...
	function _create(&$parent, $name, $shortname, $dynamic_template, $admin_url, $owner_id, $class_id, &$preparedFields, $predefinedID = -1) {
		$new_id = -1;
	...
	function removeNodeFromCache($nc_ID_or_PATH) {
	...
	function& createWithDefaultValues(&$parent, $name, $shortname, $dynamic_template, $admin_url, $owner_id, $class_id) {
		global $NULL;
	...
	function& createWithDefaultValues_rnull(&$parent, $name, $shortname, $dynamic_template, $admin_url, $owner_id, $class_id) {
		global $NULL;
		if (($parent == null) && Node::hasRoot()) {
			return null;
		}
	...
	function& createWithDefaultValues0($parent_path, $name, $shortname, $class_shortname) {
		global $NULL;
	...
	function& createWithDefaultValues0AndID($id, $parent_path, $name, $shortname, $class_shortname) {
		global $NULL;
		$parent = &Node::findByPath($parent_path);
		if (($parent == null) && Node::hasRoot()) {
			return null;
	...
	function& createWithPreparedValues(&$parent, $name, $shortname, $dynamic_template, $admin_url, $owner_id, $class_id, &$fields) {
		if (($parent == null) && Node::hasRoot()) {
			return null;
		}
	...
	function findAbsolutePathById($id) {
		global $_NODES_CACHE_BY_ID, $_NODES_CACHE_BY_PATH, $_USE_NODES_CACHE_;
	...
	function& selectNodesNSQLOnly($sql, $params) {
		global $connection;
	...
	function& findByQuery($dbmQL, $params = array(), $offset=-1, $limit=-1, $groupId=-1) {
	...
	function findCountByQuery($dbmQL, $params = array(), $groupId=-1) {
		$query = & new NodeQuery($dbmQL, $groupId);
	...
	function getChildrenCountOfClass($class_id_or_nc) {
		...
		return DBUtils::execCountSelect("SELECT COUNT(*) FROM dbm_nodes WHERE parent_id=? AND class_id=?", $params);
	}
	...
	function getCountOfRecursiveChildrenOfClasses($classes) {
		$class_ids = array();
		foreach($classes as $c) {
			$nc = &_getNodeClass($c);
			$class_ids[] = $nc->id;
	...
	function getCountOfRecursiveChildrenOfClassesForGroup($groupId, $classes) {
		$class_ids = array();
		foreach($classes as $c) {
			$nc = &_getNodeClass($c);
			$class_ids[] = $nc->id;
	...
	function recursiveUpdateSubmission(&$node, $new_parent_id) {
	...
	function& findNodesByFullTextSearch($query) {
		$nodes = array();
		$node_ids = array();
